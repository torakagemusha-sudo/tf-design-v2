...
export interface CellFormatterJsonProps extends TorafirmaComponentBaseProps {
...
const CellFormatterJson: React.FC<CellFormatterJsonProps> = ({
  id,
  testId,
  value,
  maxLines = 3,
...
') + '...' : json;
  return (
    <span id={id} data-testid={testId} data-state={state} data-trace-id={traceId} className="tf-cell-formatter-json">
      ...
      {lines.length > maxLines && !expanded && <button className="tf-cell-formatter-json__expand" onClick={onExpand}>more</button>}
    </span>
  );
};

CellFormatterJson.displayName = 'CellFormatterJson';
export default CellFormatterJson;
